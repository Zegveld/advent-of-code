...
public class Day01 extends DayTemplate {
	...
	long part1() {
		int endFloor = getInput().flatMapToInt(String::chars).reduce(0, this::reduce);
		return endFloor;
	}

	int toFloorChangeCount(String line) {
		return line.chars().map(this::toDirection).sum();
	}

	private int toDirection(int c) {
		return c == '(' ? +1 : c == ')' ? -1 : 0;
	}
	 ...
	 */
	long part2() {
		getInput().flatMapToInt(String::chars).reduce(0, this::reduce);
		return firstIndexAtBasement;
	}

	int firstIndexAtBasement = 0;
	boolean visitedBasement = false;

	int reduce(int current, int next) {
		if (!visitedBasement) {
			firstIndexAtBasement++;
		}
		int direction = toDirection(next);
