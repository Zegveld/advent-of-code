...
public class Day20 extends DayTemplate {
	 ...
	 *
	 * Your puzzle input is 36_000_000.
	 *
	 * </pre>
	 */
	public int part1() {
		int presents = 0;
		int currentHouse = 0;
		while (presents < 36_000_000) {
			currentHouse++;
			presents = calculatePresents(currentHouse);
		}
		return currentHouse;
	}

	private int calculatePresents(int currentHouse) {
		return (currentHouse + divSum(currentHouse)) * 10;
	}
	...
	public int part2() {
		...
		while (presents < 36_000_000) {
			currentHouse++;
			presents = calculatePresentsOnly50(currentHouse);
		}
		return currentHouse;
	}

	private int calculatePresentsOnly50(int currentHouse) {
		return (currentHouse + divSumOnly50(currentHouse)) * 11;
	}

	// Function to calculate sum of all proper
	// divisors num --> given natural number
	static int divSumOnly50(int num) {
		// Final result of summation of divisors
		int result = 0;
