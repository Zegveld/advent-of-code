...
public class Day02 extends DayTemplate {
	 ...
	 *
	 * Fortunately, every present is a box (a perfect right rectangular prism), which makes calculating the required wrapping paper for each
	 * gift a little easier: find the surface area of the box, which is 2*l*w + 2*w*h + 2*h*l. The elves also need a little extra paper for
	 ...
	 */
	long part1() {
		return getInput().mapToLong(this::calculatePaper).sum();
	}

	int calculatePaper(String packageLwh) {
		String[] splitted = packageLwh.split("x");
		int l = Integer.parseInt(splitted[0]);
		int w = Integer.parseInt(splitted[1]);
		int h = Integer.parseInt(splitted[2]);
		if (l == 0 || w == 0 || h == 0) {
			return 0;
		}
		int lw = l * w;
		int lh = l * h;
		int wh = w * h;
		int smallest = lw <= wh && lw <= lh ? lw : wh <= lw && wh <= lh ? wh : lh;
	 ...
	 */
	long part2() {
		return getInput().mapToLong(this::calculateLint).sum();
	}

	int calculateLint(String packageLwh) {
		String[] splitted = packageLwh.split("x");
		int l = Integer.parseInt(splitted[0]);
		int w = Integer.parseInt(splitted[1]);
		int h = Integer.parseInt(splitted[2]);
		...
		int largest = l >= h && l >= w ? l : w >= l && w >= h ? w : h;
		return (l + w + h - largest) * 2 + l * w * h;
	}
