...
public class Day08 extends DayTemplate {
	 ...
	 * escape sequences used are \\ (which represents a single backslash), \" (which represents a lone double-quote
	 ...
	 */
	public int part1() {
		return getInput().mapToInt(this::countCharacterDifference).sum();
	}
	 ...
	 */
	public int part2() {
		return getInput().mapToInt(this::countCharacterIncrease).sum();
	}

	int countCharacterDifference(String string1) {
		String filtered = string1.replaceAll("^\"", "");
		filtered = filtered.replaceAll("\"$", "");
		filtered = filtered.replaceAll("\\\\(\\W)", "$1");
		filtered = filtered.replaceAll("\\\\x[\\da-f]{2}", "#");
		return string1.length() - filtered.length();
	}

	int countCharacterIncrease(String string1) {
		String encoded = '"' + string1.replaceAll("(\\W)", "\\$1") + '"';
		return encoded.length() - string1.length();
	}
